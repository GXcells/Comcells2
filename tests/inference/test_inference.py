...
from comcells2.samplers import KSampler
...
#
comcells2_graph_list = [DEFAULT_COMFY_GRAPH]
comcells2_graph_ids = [DEFAULT_COMFY_GRAPH_ID]
prompt_list = [
    'a painting of a cat',
]
...
class TestInference:
    ...
    def start_client(self, listen:str, port:int):
        # Start client
        comcells2_client = ComfyClient()
        # Connect to server (with retries)
        n_tries = 5
        for i in range(n_tries):
            time.sleep(4)
            try:
                comcells2_client.connect(listen=listen, port=port)
            except ConnectionRefusedError as e:
                print(e)
                print(f"({i+1}/{n_tries}) Retrying...")
            else:
                break
        return comcells2_client
    ...
    @fixture(scope="class", params=comcells2_graph_list, ids=comcells2_graph_ids, autouse=True)
    def _client_graph(self, request, args_pytest, _server) -> (ComfyClient, ComfyGraph):
        comcells2_graph = request.param
        
        # Start client
        comcells2_client = self.start_client(args_pytest["listen"], args_pytest["port"])

        # Warm up pipeline
        comcells2_client.get_images(graph=comcells2_graph.graph, save=False)

        yield comcells2_client, comcells2_graph
        del comcells2_client
        del comcells2_graph
        torch.cuda.empty_cache()
    ...
    @fixture
    def comcells2_graph(self, _client_graph):
    ...
    def test_comcells2(
        ...
        comcells2_graph.set_filename_prefix(test_info)
        # Settings for comcells2 graph
        comcells2_graph.set_sampler_name(sampler)
        comcells2_graph.set_scheduler(scheduler)
        comcells2_graph.set_prompt(prompt)

        # Generate
        images = client.get_images(comcells2_graph.graph)
