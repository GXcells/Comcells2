    ...
    def __init__(self, loop):
        ...
        async def view_metadata(request):
            ...
            if safetensors_path is None:
                return web.Response(status=404)
            out = comcells2.utils.safetensors_header(safetensors_path, max_size=1024*1024)
            if out is None:
                return web.Response(status=404)
            dt = json.loads(out)
        ...
        async def get_queue(request):
            ...
            vram_total, torch_vram_total = comcells2.model_management.get_total_memory(device, torch_total_too=True)
            vram_free, torch_vram_free = comcells2.model_management.get_free_memory(device, torch_free_too=True)
            system_stats = {
                "system": {
                    "os": os.name,
